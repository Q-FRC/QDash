...
Control {
    ...
    */
    function setValue(newValue, noWrap, notModified) {
        ...
        newValue = Number(newValue.toFixed(Math.max(decimals, 0)))
        // round
        ...
        if (value !== newValue) {
            ...
            if (spinBoxItem)
                spinBoxItem.value = 0
            // reset this to prevent it from disabling the buttons or other weirdness
            return true
        }
        return false
    }
    ...
    */
    ...
    function updateValueFromText() {
        if (!setValue(textValue(), true))
            updateUi()
        // make sure the text is formatted anyway
    }
